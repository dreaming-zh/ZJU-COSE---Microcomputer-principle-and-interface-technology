...
u8	disp_index	= 0, F_Dir = 1; /* F_Dir方向标记，0-左移 1-不动 2-右移 */

u8 code disp_string[] = "3200104869 ";

u16 speed=500; //显示速度，单位ms，分为500 1000 2000三档
...
void ModifyRTC(void)
{
	...
	if(!interpret()) {	 
		printf("输入格式为\"设置 xx\"\n");
		printf("前一个取值为012 对应方向 后一个x对应速度，0是0.5s 1是1s 2是2s\n");	
		return;
	} 
	ReflashDisp();
}
