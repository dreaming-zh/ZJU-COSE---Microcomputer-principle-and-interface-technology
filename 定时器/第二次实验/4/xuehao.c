...
{
u16 i=0;
u8 xuehao[10]={3,2,0,0,1,0,4,8,6,9};
Port_Init(); //初始化
T0_Init();
INT_Init();
...
if(F_stop==1){i++;F_stop=0;}
	
if(F_incdec==1){i--;F_incdec=0;}
	
LED8[0]=xuehao[(0+i)%10];
LED8[1]=xuehao[(1+i)%10];
LED8[2]=xuehao[(2+i)%10];
LED8[3]=xuehao[(3+i)%10];
LED8[4]=xuehao[(4+i)%10];
LED8[5]=xuehao[(5+i)%10];
LED8[6]=xuehao[(6+i)%10];
LED8[7]=xuehao[(7+i)%10];
